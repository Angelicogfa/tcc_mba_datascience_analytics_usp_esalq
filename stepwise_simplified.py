        ...
        try:
            ...
            if not convergence_valid:
                if self.require_convergence:
                    ...
                    self._convergence_stats['failed_convergence'] += 1
                    if self.verbose:
                        print(f"   Baseline model rejected: convergence requirements not met")
                    return None
                else:
                    # Se convergência não é obrigatória, apenas avisar
                    if self.verbose:
                        print(f"   Warning: baseline model did not meet convergence requirements but accepted")
            else:
                self._convergence_stats['converged_fits'] += 1
            ...
            self._convergence_stats['numerical_issues'] += 1
            if self.verbose:
                print(f"Error fitting baseline model: {str(e)[:50]}...")
            return None
